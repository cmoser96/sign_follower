...
class StreetSignRecognizer(object):
    ...
    def __init__(self):
        ...
        # Creates a window for changing the bounds
        cv2.namedWindow('threshold_image')
        self.hsv_lb = np.array([0, 0, 0]) # hsv lower bound
        cv2.createTrackbar('H lb', 'threshold_image', 23, 255, self.set_h_lb)
    ...
    def process_image(self, msg):
        ...
        self.filtered = cv2.inRange(self.hsv_image, self.hsv_lb, self.hsv_ub)
    ...
    def sign_bounding_box(self):
        ...
        points = None
        cv2.findNonZero(self.filtered, points)
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            if not self.cv_image is None:
                ...
                cv2.imshow('bgr_window', self.cv_image)
                cv2.waitKey(5)

            if not self.filtered is None:
                # creates a window and displays the image for X milliseconds
                cv2.imshow('hsv_window', self.filtered)
                cv2.waitKey(5)

            if not self.image_info_window is None:
                # creates a window and displays bgr/hsv values
